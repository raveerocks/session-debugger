...
import { getSessionLogsSummary } from '../../controllers/SessionLogsController';
...
const SessionSummary = (props) => {
  ...
  useEffect(() => {
    getSessionLogsSummary(props.category, props.session)
      .then((response) => {
              ...
              <Column>
                <VTableCard>
                  ...
                  <VRowCard>
                    ...
                    <VColumnCard>{`${(result.seesion_duration / 1000).toFixed(
                      3
                    )} s`}</VColumnCard>
                  </VRowCard>
